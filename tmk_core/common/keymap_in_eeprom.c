...
void keymap_in_eeprom_disable(void) {
    eeprom_update_word(EECONFIG_KEYMAP_CHECKSUM, eeprom_read_word(EECONFIG_KEYMAP_CHECKSUM) + 1);
}
...
#ifdef DEBUG
    eeprom_update_word(EECONFIG_KEYMAP_DEBUG, checksum);
#endif
...
void write_keymap_to_eeprom(void) {
    ...
    // write checksum
    eeprom_update_word(EECONFIG_KEYMAP_CHECKSUM, checksum);
}
...
void eeconfig_write_keymap_fn_action(uint8_t index, uint16_t fn_action) {
    return eeprom_update_word(EECONFIG_KEYMAP_FN_ACTIONS + index, fn_action);
}
...
void eeconfig_write_keymap_key(uint8_t layer, uint8_t row, uint8_t col, uint8_t key) {
    return eeprom_update_byte(EECONFIG_KEYMAP_KEYMAPS + KEY_OFFSET(layer, row, col), key);
}
...
void eeconfig_write_keymap_key_by_index(uint16_t index, uint8_t key) {
    return eeprom_update_byte(EECONFIG_KEYMAP_KEYMAPS + index, key);
}
